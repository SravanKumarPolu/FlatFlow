...
export const memberFormSchema = z.object({
  ...
  emoji: z.string().max(2, "Emoji must be 1-2 characters").optional().or(z.literal("")),
  weight: z.number().positive("Weight must be a positive number").min(0.1, "Weight must be at least 0.1"),
  isActive: z.boolean(),
});
...
export const billFormSchema = z.object({
  ...
  dueDay: z.number().int("Due day must be a whole number").min(1, "Due day must be between 1 and 31").max(31, "Due day must be between 1 and 31"),
...
export const expenseFormSchema = z.object({
  ...
  category: z.enum(["RENT", "UTILITY", "FOOD", "TRAVEL", "GROCERY", "SWIGGY", "OLA_UBER", "OTHER"]),
  paidByMemberId: z.string().min(1, "Please select who paid"),
  participantMemberIds: z.array(z.string()).min(1, "At least one participant is required"),
});
...
export const flatFormSchema = z.object({
  ...
  billingCycleStartDay: z.number().int("Billing cycle day must be a whole number").min(1, "Billing cycle day must be between 1 and 28").max(28, "Billing cycle day must be between 1 and 28"),
...
export const choreFormSchema = z.object({
  ...
  category: z.enum(["CLEANING", "KITCHEN", "BATHROOM", "TRASH", "UTILITIES", "OTHER"]),
